...
async def test_parse():
    class MockManager:
        async def broadcast(self, conv_id, msg):
            print("BROADCAST:", repr(msg['content']))
...
```"""
    ...
    # We purposefully make chunk_size such that it cuts exactly at `<`
    chunk_size = 512
    raw_bytes = raw_text.encode('utf-8')
    
    # First chunk, process normally
    i = 0
    while i < len(raw_bytes):
        ...
        # Suppose a chunk ends exactly at: "-                        if times['sub'] <"
            ...
            if not in_thought:
                ...
                if t_idx != -1 or tk_idx != -1:
                    ...
                    if possible_partial:
                        ...
                        break
        i += chunk_size
        if "if times['sub'] <" in buffer:
            print("Buffer stuck at:", repr(buffer))
